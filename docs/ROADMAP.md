...
> **Goal**: Type-aware TypeScript code generation that significantly exceeds the **Qwen3-Coder-8B baseline** on type-correctness metrics.

---

## Strategy

We do **not** aim to beat the absolute state of the art (Copilot, GPT-4, Qwen3-Coder-32B, etc.).
We aim to show that adding **type prediction + hallucination control** on top of a fixed, publicly available baseline (Qwen3-Coder-8B) produces a measurable improvement on type-related metrics — without regression on general code-generation quality.
...
- **[Qwen3-Coder-8B-Instruct](https://huggingface.co/Qwen/Qwen3-Coder-8B-Instruct)** (Apache 2.0)
- Reasons:
  - Strong TypeScript baseline (Qwen3-Coder family is current SOTA in 7–14B class)
...
Three components added on top of the frozen Qwen3-Coder backbone (LoRA-trained):
...
- Load Qwen3-Coder-8B-Instruct, run baseline on the evaluation suite
...
- Compare yamatoLLM vs Qwen3-Coder-8B baseline on the metrics below
